...
class SiteController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $news = PublishInfo::find()->orderBy(['publish_time' => SORT_DESC])->limit(5)->all();
        $messages = MessageBoard::find()->orderBy(['msg_time' => SORT_DESC])->limit(5)->all();
    ...
    public function actionMessageBoard()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            Yii::$app->session->setFlash('success', '留言已提交，感谢你的分享。');
            return $this->refresh();
        }

        $messages = MessageBoard::find()->orderBy(['msg_time' => SORT_DESC])->all();
